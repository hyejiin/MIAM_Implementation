...
def normalize(data, data_mask, mean, std):
    ...
    for v in range(n_variables):
        # 0이 아닌 값의 인덱스 선택
        idx = np.where((mask[:, v] == 1) & (measure[:, v] != 0))[0]

        # 0이 아닌 값이 없으면 해당 변수는 건너뜀
        if len(idx) == 0:
            continue

        if isnew:
            # 0이 아닌 값들로 평균과 표준편차 계산
        ...
        # 0이 아닌 값들에 대해서만 정규화 수행
        for ix in idx:
            if measure_std != 0:
                measure[ix, v] = (measure[ix, v] - measure_mean) / measure_std
            else:
                measure[ix, v] = measure[ix, v] - measure_mean
...
def collate_fns(recs):
    rec_dict = {'values': torch.FloatTensor(np.array([r['values'] for r in recs])),
                ...
                'times': torch.FloatTensor(np.array([r['times'] for r in recs])),
                'labels': torch.FloatTensor(np.array([r['labels'] for r in recs])),
                'los_labels': torch.FloatTensor(np.array([r['los_labels'] for r in recs]))
                }
    return rec_dict

# Define Sample Loader
def sample_loader(phase, k, data, mask, label, label2, time, batch_size, ZeroImpute):
    # Random seed
    manualSeed = 128
    ...
    #if D ==35: #Physionet
    with open('/media/usr/SSD/hyejin/kfold_data.pkl', 'rb') as f:
        kfold_data = pickle.load(f)
        
    if phase == 'train':
        times = kfold_data['kfold_Delta'][k][0]
    elif phase == 'valid':
        times = kfold_data['kfold_Delta'][k][1]
    elif phase == 'test':
        times = kfold_data['kfold_Delta'][k][2]
    ...
    for i in range(N):
        ...
        rec = {}
        rec['labels'] = label[i]
        rec['los_labels'] = label2[i]
        if ZeroImpute:
            values[np.isnan(values)] = 0
            rec['values'] = np.nan_to_num(values).tolist()
            rec['masks'] = masks.astype('int32').tolist()
            rec['deltas'] = times[i].tolist()
            # rec['deltas'] = z_times[i].tolist()
...
def sample_loaders(phase, k, data, mask, label, label2, label3, time, batch_size, ZeroImpute):
    # Random seed
    manualSeed = 128
    np.random.seed(manualSeed)
    torch.manual_seed(manualSeed)
    ...
    times = np.zeros((N, T, D))
    if D ==35:
        times = np.load(open('/home/yrlee/MultiMHA/Data/kfold_delta_35_' + str(k) + '_' + phase + '.npz', 'rb'), mmap_mode='r', allow_pickle=True)['delta'] #physionet #/home/yurimlee/Project/EHR_Analytics/MultiMHA/Data/kfold_delta_35_
    else:
        times = np.load(open('/home/yrlee/MultiMHA/Data/kfold_delta_mimic_' + str(k) + '_' + phase + '.npz', 'rb'), mmap_mode='r', allow_pickle=True)['delta'] #mimic
...
def msk_sample_loader(phase, k, data, mask, msk_data, mmsk,  label, label2, time, batch_size, ZeroImpute):
    # Random seed
    manualSeed = 128
    np.random.seed(manualSeed)
    torch.manual_seed(manualSeed)
...
#         times = np.load(open('./Data/kfold_delta_mimic_' + str(k) + '_' + phase + '.npz', 'rb'), mmap_mode='r', allow_pickle=True)['delta'] #mimic
    ...
    for i in range(N):
        ...
        vals = data[i]
        mskval = msk_data[i]
        values = vals  # (T, D) 형태 유지
        masks = mask[i]  # (T, D) 형태 유지
...
def msk_sample_loaders(phase, k, data, mask, msk_data, mmsk, label, label2, label3, time, batch_size, ZeroImpute):#data, data_mask, org, label, batch_size):
    # Random seed
    manualSeed = 128
    np.random.seed(manualSeed)
    torch.manual_seed(manualSeed)
    ...
    for i in range(N):
        ...
        values = np.vstack([vals, mskval]).transpose(1,2,0)
        msk = np.expand_dims(mask[i], axis=0)
        mmask = np.expand_dims(np.tile(np.expand_dims(mmsk[i], axis=1), 35), axis=0)
        masks = np.vstack([msk, mmask]).transpose(1,2,0)
        ...
        if ZeroImpute:
            ...
            rec['values'] = np.nan_to_num(values).tolist()
            rec['masks'] = masks.astype('int32').tolist()
            rec['rmasks'] = mmask.astype('int32').tolist()
            rec['deltas'] = times[i].tolist()
            rec['times'] = ori_time[i].tolist()
        else:
            rec['values'] = values.tolist()
...
class FocalLoss(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        if self.logits:
            # Convert logits to probabilities if needed for binary_cross_entropy
            BCE_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none')
        else:
            # inputs should be probabilities in this case (0 <= inputs <= 1)
