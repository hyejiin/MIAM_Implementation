...
def backprop_lrp_fc(weight, bias, activations, R, pn = 'p1',
    ...
    if pn == 'p':
        ...
        s_p = R / z_p
        c_p = torch.matmul(s_p.T, weight_p)
        R_c = activations * c_p
        ...
        z_p = torch.matmul(activations, weight_p.T) + bias_p + eps
        s_p = R / z_p
        c_p = torch.matmul(s_p, weight_p.T)
        R_c = activations * c_p
        ...
        z_n = torch.matmul(weight_n, activations.T) + bias_n - eps
        s_n = R / z_n
        c_n = torch.matmul(s_n.T, weight_n)
        R_c = activations * c_n
    # R_c = rescale_lrp(R, torch.mean(R_c, axis=1))
